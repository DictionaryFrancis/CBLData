...
app.title('Welcome to CBLData')
# store email address and password
test_pdf_name = tk.StringVar()


def handle_test():
    pdf_sei = test_pdf_name.get()
    print(pdf_sei)
    team_name = options.get()
    print(team_name)


# label
label_options = ttk.Label(text="Please select the team:")
label_options.pack(padx=5, pady=10)

# combobox
options = ttk.Combobox(app, values=["Helio Inter Cork\n",
                                    "LongBoats\n",
                                    "SCS Crookstown Utd\n",
                                    "OBS\n",
...
# email
email_label = ttk.Label(signin, text="Insert the pdf name:")
email_label.pack(fill='x', expand=True)

email_entry = ttk.Entry(signin, textvariable=test_pdf_name)
email_entry.pack(fill='x', expand=True)
email_entry.focus()

# login button
login_button = ttk.Button(signin, text="Enter", command=handle_test)
